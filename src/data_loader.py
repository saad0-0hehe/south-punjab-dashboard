...
NUMERIC_COLUMNS = [
    "population_2023", "area_sqkm", "density_per_sqkm",
...
def load_data(filepath=None):
    ...
    print(f"✅ Loaded {len(df)} rows × {len(df.columns)} columns from {os.path.basename(filepath)}")
    return df
...
def get_summary_stats(df, group_col="region"):
    ...
    key_indicators = [
        ...
        "mpi_score", "primary_enrollment_rate", "immunization_coverage",
        "clean_water_access", "gender_literacy_gap"
    ]
